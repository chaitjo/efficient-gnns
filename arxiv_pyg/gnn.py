...
class ProjectionGCD(torch.nn.Module):
    def __init__(self, hidden_channels, proj_dim):
        ...
        self.lin = torch.nn.Linear(hidden_channels, proj_dim)
        self.conv = GCNConv(hidden_channels, proj_dim)
        self.bn = torch.nn.BatchNorm1d(proj_dim)
    
    def forward(self, x, adj_t):
        x = self.lin(x) + self.conv(x, adj_t)
        x = self.bn(x)
        x = F.relu(x)
        return x
...
def train(model, data, train_idx, optimizer, args, teacher_out_feat, teacher_logits, student_proj=None, teacher_proj=None, edge_index=None):
    ...
    elif args.training == 'crd':
        out_feat = student_proj(model.out_feat[train_idx])
        teacher_out_feat = teacher_proj(teacher_out_feat[train_idx])
        loss, loss_cls, loss_aux = crd_criterion(
            out, labels, out_feat, teacher_out_feat, 
    ...
    elif args.training == 'nce-labels':
        out_feat = student_proj(model.out_feat[train_idx])
        teacher_out_feat = teacher_proj(teacher_out_feat[train_idx])
        loss, loss_cls, loss_aux = nce_criterion_with_labels(
            out, labels, out_feat, teacher_out_feat, 
            args.beta, args.nce_T, args.max_samples
        )
    elif args.training == 'nce-edges':
        out_feat = student_proj(model.out_feat[train_idx])
        teacher_out_feat = teacher_proj(teacher_out_feat[train_idx])
        loss, loss_cls, loss_aux = nce_criterion_with_edges(
            out, labels, out_feat, teacher_out_feat, 
            edge_index, args.beta, args.nce_T, args.max_samples
        )
    elif args.training == 'nce-labels-edges':
        out_feat = student_proj(model.out_feat[train_idx])
        teacher_out_feat = teacher_proj(teacher_out_feat[train_idx])
        loss, loss_cls, loss_aux = nce_criterion_with_labels_and_edges(
            out, labels, out_feat, teacher_out_feat, 
            edge_index, args.beta, args.nce_T, args.max_samples
        )
...
def main():
    ...
    for run in range(args.runs):
        ...
        if args.training in ["nce", "fitnet", "gpw", "nce-labels", "nce-edges", "nce-labels-edges", "crd", "gcd"]:
