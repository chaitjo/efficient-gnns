...
@torch.no_grad()
def evaluate(args, model, graph, labels, train_idx, val_idx, test_idx, evaluator, device):
    model.eval()
...
def main():
    ...
    argparser.add_argument("--checkpoint-files", type=str, default="./checkpoints/gat-3L250x3h/*.pt", help="address of checkpoint files")
    ...
    for file in glob.iglob(args.checkpoint_files):
        ...
        train_acc, val_acc, test_acc, train_loss, val_loss, test_loss, pred, model_feat, iter_time, gpu_used = evaluate(
            args, model, graph, labels, train_idx, val_idx, test_idx, evaluator_wrapper, args.gpu
        )
...
# python test_gat.py --use-norm --use-labels --n-label-iters=1 --no-attn-dst --edge-drop=0.3 --input-drop=0.25 --gpu 0