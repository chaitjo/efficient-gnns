...
def evaluate(args, model, graph, labels, train_idx, val_idx, test_idx, evaluator):
    ...
    model_feat = model.feat  # final hidden features before prediction
...
def run(args, graph, labels, train_idx, val_idx, test_idx, evaluator, n_running):
    ...
    if args.save_pred:
        os.makedirs(f"./output/{args.expt_name}", exist_ok=True)
        torch.save(F.softmax(final_pred, dim=1), f"./output/{args.expt_name}/{n_running}.pt")

        os.makedirs(f"./logits/{args.expt_name}", exist_ok=True)
        torch.save(final_pred, f"./logits/{args.expt_name}/{n_running}.pt")

        os.makedirs(f"./features/{args.expt_name}", exist_ok=True)
        torch.save(final_feat, f"./features/{args.expt_name}/{n_running}.pt")

        os.makedirs(f"./checkpoints/{args.expt_name}", exist_ok=True)
        torch.save({
            'args': args,
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict()
        }, f"./checkpoints/{args.expt_name}/{n_running}.pt")
...
def main():
    ...
    argparser.add_argument("--save-pred", action="store_true", help="save final predictions and features")
    argparser.add_argument("--expt-name", type=str, default='debug', help="directory to save results")
